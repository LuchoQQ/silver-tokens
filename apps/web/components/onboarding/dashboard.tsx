...
export function OnboardingDashboard({ user, mcpCommand, token, tokenExpires, signInAction }: OnboardingProps) {
  const isSignedIn = !!user && !!mcpCommand && !!token;
  const firstName = user?.name?.split(' ')[0] ?? 'there';

  return (
    <div className="app">
      <header className="appbar">
        <Link href="/" className="brand"><BrandSvg /></Link>
        <nav className="nav">
          {isSignedIn ? (
            <>
              <Link href="/">My scorecard</Link>
              <a href="#" className="active">Setup</a>
            </>
          ) : (
            <a href="#" className="active">Setup</a>
          )}
      ...
      <main className="page">
        <div className="onboarding">
          <div className="hero">
            <h1>
              {isSignedIn ? (
                ...
                <><span style={{ color: 'var(--primary)' }}>Silver</span> Tokens</>
              )}
            </h1>
            <p>
              {isSignedIn
                ? "Two pastes and you're done. We'll start ingesting your local CLI history within ~10 seconds."
                : 'Sign in with GitHub to get a token, then paste two commands to unlock your scorecard.'}
            </p>
          </div>

          <div className={`step ${isSignedIn ? 'done' : 'active'}`}>
            <div className="num">
              {isSignedIn ? (
            ...
            <div>
              ...
              {isSignedIn ? (
                ...
                <>
                  <p>We use your GitHub login to associate uploads with your candidate profile. No repo permissions requested.</p>
                  {signInAction && (
                    <form action={signInAction}>
                      <button type="submit" className="btn btn--default btn--sm" style={{ marginTop: 8 }}>
          ...
          <div className={`step ${isSignedIn ? 'active' : ''}`}>
            <div className="num">2</div>
            <div>
              <h4>Add the Silver MCP to your CLI</h4>
              <p>Paste this into a terminal where Claude Code, Codex, or OpenCode is installed. The same command works for all three.</p>
              {isSignedIn && mcpCommand ? (
                <div className="cmd">
                  <span style={{ color: 'oklch(0.7 0.14 45)' }}>$</span>
                  <span style={{ color: 'oklch(0.985 0 0)' }}>{mcpCommand}</span>
                </div>
              ) : (
                <p style={{ fontSize: 12, marginTop: 10, marginBottom: 0, color: 'var(--muted-foreground)' }}>
                  Sign in first — your personal command appears here.
                </p>
              )}
              {isSignedIn && (
          ...
          <div className="step">
            ...
            <div>
              <h4>
                Run{' '}
                <code style={{ fontFamily: 'ui-monospace, monospace', background: 'var(--muted)', padding: '2px 6px', borderRadius: 4, fontSize: 13 }}>
                  /mcp__silver__track
                </code>
              </h4>
              <p>Inside the CLI, run that slash command once. We&apos;ll scan your local history (sanitized — no prompts or code), upload it, and unlock your scorecard.</p>
              {isSignedIn && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            <div>
              <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted-foreground)', fontWeight: 600, margin: '0 0 4px' }}>Privacy</p>
              <p style={{ fontSize: 13, margin: 0, color: 'var(--muted-foreground)', lineHeight: 1.5 }}>
                Token counts, model, timestamps. <strong style={{ color: 'var(--foreground)' }}>Never</strong> prompts, code, or file contents.
              </p>
            </div>
            <div>
              <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted-foreground)', fontWeight: 600, margin: '0 0 4px' }}>Idempotent</p>
              <p style={{ fontSize: 13, margin: 0, color: 'var(--muted-foreground)', lineHeight: 1.5 }}>
                Re-run the command anytime. Duplicates are deduped server-side.
              </p>
            </div>
            <div>
              <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted-foreground)', fontWeight: 600, margin: '0 0 4px' }}>Token</p>
              <p style={{ fontSize: 13, margin: 0, color: 'var(--muted-foreground)', lineHeight: 1.5, wordBreak: 'break-all', fontFamily: 'ui-monospace, monospace' }}>
                {token ?? '—'}
              </p>
            </div>
          </div>
